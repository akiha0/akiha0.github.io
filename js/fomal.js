...
$.ajax({
  ...
  data: {
    key: 'WYTBZ-U7AW4-SWEUE-FUIWN-VD5RV-LIBDB',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(113.2561249, 23.0868850, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime1() {
  var grt = new Date("07/20/2024 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by AKIHA %c 你正在访问AKIHAの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| AKIHA") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('AKIHA的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("07/20/2024 00:00:00"); // 旅行者1号开始计算的时间
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    ...
    --darkmode-bg: url(https://source.fomal.cc/img/dark_mode.webp);
    --mobileday-bg: url(https://source.fomal.cc/img/mb12.webp);
    --mobilenight-bg: url(https://source.fomal.cc/img/mb14.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
